...
const flags = [
  { label: "big js disabled", features: { bigjs: false } },
  { label: "big js enabled", features: { bigjs: true } },
];

for (const flag of flags) {

  describe(`CSV App Test Suite - Feature (${flag.label})`, () => {

    const client = setupTest(__dirname, "./resources/csv-app");

    const cds = cwdRequireCDS();
    cds.env.log.levels.db = "debug";
    Object.assign(cds.env.features, flag.features);
